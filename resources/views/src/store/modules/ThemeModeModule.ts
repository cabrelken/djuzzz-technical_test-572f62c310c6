...
export default class ThemeModeModule extends VuexModule implements StoreInfo {
  ...
  @Mutation
  [Mutations.SET_ASIDE_MODE_MUTATION](payload) {
    this.mode = payload;
  }

  @Mutation
  [Mutations.SET_HEADER_MODE_MUTATION](payload) {
    this.mode = payload;
  }
  ...
  @Action
  [Actions.SET_ASIDE_MODE_ACTION](payload) {
    localStorage.setItem(themeModeLSKey, payload);
    localStorage.setItem(themeMenuModeLSKey, payload);
    document.documentElement.setAttribute("data-theme", payload);
    ThemeModeComponent.init();
    this.context.commit(Mutations.SET_ASIDE_MODE_MUTATION, payload);
  }

  @Action
  [Actions.SET_HEADER_MODE_ACTION](payload) {
    localStorage.setItem(themeModeLSKey, payload);
    localStorage.setItem(themeMenuModeLSKey, payload);
    document.documentElement.setAttribute("data-theme", payload);
    ThemeModeComponent.init();
    this.context.commit(Mutations.SET_HEADER_MODE_MUTATION, payload);
  }
}
